...
func NewWalListener(
	...
	return &Listener{
		slotName:   fmt.Sprintf("%s_%s", cfg.GetString("listener.slotName"), cfg.GetString("database.name")),
		config:     cfg,
		publisher:  publ,
		repository: repo,
		replicator: repl,
...
func (l *Listener) Process(ctx context.Context) error {
	...
	logger.WithField("logger_level", l.config.GetString("logger.level")).Infoln(StartServiceMessage)
	...
	refreshConnection, _ := time.ParseDuration(l.config.GetString("listener.refreshConnection"))
	refresh := time.NewTicker(refreshConnection)
...
const (
	...
	publicationName = "pgwalstreams"
)

// FilterTables generate the tables filter from configmap
func FilterTables(cfg *viper.Viper) config.FilterStruct {

	var filterTables config.FilterStruct

	if err := cfg.Sub("database.filter").Unmarshal(&filterTables); err != nil {
		logrus.Fatalln(err)
	}
...
func (l *Listener) Stream(ctx context.Context) {
	...
	filterTables := FilterTables(l.config)

	l.config.OnConfigChange(func(e fsnotify.Event) {
		logrus.WithField("file", e.Name).Infoln("Config file changed")
		filterTables = FilterTables(l.config)
	})
	l.config.WatchConfig()
		...
		if msg != nil {
			if msg.WalMessage != nil {
				...
				if tx.CommitTime != nil {
					natsEvents := tx.CreateEventsWithFilter(filterTables.Tables)
					for _, event := range natsEvents {
						subjectName := event.GetSubjectName(l.config.GetString("nats.topicPrefix"))
...
func (l *Listener) SendPeriodicHeartbeats(ctx context.Context) {
	heartbeatInterval, _ := time.ParseDuration(l.config.GetString("listener.heartbeatInterval"))
	heart := time.NewTicker(heartbeatInterval)
	defer heart.Stop()
